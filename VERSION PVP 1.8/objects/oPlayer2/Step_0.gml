...
var key_melee = keyboard_check(ord("P"));
...
// Identify as dead
if (respawn_timer == 0 && is_dead == false && oGamePVP.player2_hp <= 0) {
	audio_stop_sound(aMagnetic);
	is_dead = true;
	choose_sprite(sPlayer2DieR);
	image_index = 0;
	y_speed = 0;
} 
...
// ANIMATION ************************************************************************
	...
	if (melee_started == false) {
		if (!on_floor()) {
			// Falling
			if (sign(y_speed) > 0) choose_sprite(sPlayer2FallR);	
			// Jumping
			else if (sign(y_speed) < 0) choose_sprite(sPlayer2JumpR);	
		
		} else {
			// Reset jump counter
			available_jumps = jumps_const;
			// Crouch
			if (key_down) choose_sprite(sPlayer2CrouchR);
			// Running
			else if (sign(x_speed != 0)) choose_sprite(sPlayer2RunR);
			// Idle
			else choose_sprite(sPlayer2IdleR);
		}
	...
	// If melee atk
		...
		// Crouching melee atk
		if (key_down && on_floor()) choose_sprite(sPlayer2AtkCrouchR); // melee_direction
		// Non-crouching melee atk
		else choose_sprite(sPlayer2AtkR); // melee_direction
		...
		// Finish animation
	    if (melee_current_frame >= sprite_get_number(sPlayer2AtkR)) {
	        melee_started = false;
	        melee_current_frame = 0;
			melee_animation_timer = 0;
	    }
...
if (respawn_timer == 0 && oGamePVP.game_started == false) {
	...
	y = y + y_speed; 
	if (key_down && on_floor()) {
		choose_sprite(sPlayer2CrouchR);
	} else {
		choose_sprite(sPlayer2IdleR);
	}
}
